...
### Added
- **Automation blueprint: "Annuals - Upcoming Event Reminders"** (`blueprints/automation/annuals/annual_reminders.yaml`) - target specific events or every event of a chosen type, filter by VIP and/or Important (each independently "must be" or "must NOT be", combined as AND or OR when both are active), remind at multiple "days before" thresholds in one automation. Notify via any combination of dedicated, icon'd sections: **Mobile App Notify** (one or more devices, each with a "Done" button), **Notifications** (Home Assistant's own notification bell and/or a dashboard status helper `input_text`, showing only today's reminder(s), never a stale earlier day's), **Text-to-Speech Announcement**, and **Custom Actions** (anything else via the normal action editor - email, WhatsApp/Telegram/Signal/ntfy, ...). Optionally track each due reminder as a to-do list item - due date/time and description are set automatically from the event's own date and the rendered notification message, a completed item is never recreated by a later reminder for the same event, and completing it (from the list, or via the notification's "Done" button) is what resolves the reminder either way. Notification title/message are fully customizable templates, with the event type and countdown phrase (`{{ ev_type }}`, `{{ reminder_message }}`) automatically translated into Home Assistant's configured language.
...
- **`type_label`/`reminder_message` sensor attributes** - every event sensor now exposes its event type and a ready-made "days until" countdown phrase, both already translated into Home Assistant's configured language (reusing the same translations the config flow and dashboard card already use) - primarily for the new blueprint above, but available to any automation/template.
